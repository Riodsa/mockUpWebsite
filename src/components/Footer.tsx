...
const address =
  "เลขที่ 2 ชั้น 3 อาคารเพลินจิตเซ็นเตอร์ \nถนนสุขุมวิท แขวงคลองเตย \nเขตคลองเตย กรุงเทพฯ 10110";
  ...
  return (
    <div className="relative min-w-screen h-auto flex flex-col justify-center items-center bg-(--footer-blue)">
      <div className="relative bottom-0 h-[70dvh] w-full p-10 pr-15 flex flex-col justify-between max-w-7xl self-center">
        <div className="flex flex-row justify-between relative">
          <div className="flex flex-col">
            <Image
              ...
              className="relative"
            />
            <p className="text-white whitespace-pre-line mt-5 font-(family-name:--font-thai)">
              {address}
            </p>
            <p className="text-white whitespace-pre-line mt-2 font-(family-name:--font-thai)">
              {tel}
            </p>
            <p className="text-white whitespace-pre-line mt-2 font-(family-name:--font-thai)">
              {email}
          ...
          <div className="flex w-fit">
            {navItems.map((item) => (
              <div
                className="flex flex-col ml-10 mt-10 w-35"
                key={`footer-${item}`}
              >
                <Link
                  href={navItemDict[item].href ?? "/"}
                  className="text-white pl-2 font-(family-name:--font-thai)"
                >
                  {navItemDict[item].label}
                </Link>
                <hr className="border-white my-4"></hr>
                <ul className="list-inside">
                  {navItemDict[item].items.map((subitem) => (
                    <li key={subitem.label} className="mb-2">
                      <Link href={subitem.href} className="text-white pl-2 ">
                        {subitem.label}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="flex flex-row justify-between relative">
          <p className="text-white font-(family-name:--font-thai)">
            © 2023 Mitrphol Group. All rights reserved.
          </p>
          ...
          <Link
            href="/terms"
            className="text-white font-(family-name:--font-thai)"
          >
            Privacy & Terms
          </Link>
        </div>
